...
//#if 0
	...
	if (timee > 0)
	{
		for (int l = 0, i = 0; i < 10; i++, l += 65)
		{
			for (int k = 0, j = 0; j < 15; j++, k += 65)
			{
				...
				DrawString(width / 2 - 210, height - 35, width, height, "Moves : " + Num2Str(moves), colors[BLACK]);
	...
	else
	{
		...
		DrawString(width / 2 - 200, height - 650, width, height, "Made by: ASADULLAH AFZAL", colors[WHITE]);
